...
import staticc
from staticc.delta import ChangeType, PartialChange, ast_delta

SOURCE_DIR = Path(staticc.__file__).parent
...
def test_sequence_diff(pack):
    baseline, new, changes = pack
    assert changes == [
        PartialChange(ChangeType.FIELD_SIZE, baseline, new, on_field="body")
    ]
